...
import {
  ...
  AlertCircle, Loader2, FolderPlus
} from 'lucide-react'
...
export default function UploadForm() {
  ...
  const handleUploadAll = async () => {
    ...
    for (const fw of pending) {
      // Mark as uploading
      setFiles((prev) =>
        prev.map((f) => f.file.name === fw.file.name ? { ...f, status: 'uploading' } : f)
      )
    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#1e1c32] border border-slate-700/50 rounded-2xl p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="font-semibold text-white">Name your project</h2>
        </div>
        ...
        <div className="flex gap-3">
          <input
            ...
            onKeyDown={(e) => e.key === 'Enter' && !projectCreated && handleCreateProject()}
      ...
      {/* Step 2 — Drop zone */}
      <div className={cn('bg-[#1e1c32] border border-slate-700/50 rounded-2xl p-6', !projectCreated && 'opacity-50 pointer-events-none')}>
        <div className="flex items-center gap-2 mb-4">
          <div className={cn('w-6 h-6 rounded-full text-white text-xs font-bold flex items-center justify-center', projectCreated ? 'bg-indigo-600' : 'bg-slate-600')}>2</div>
          <h2 className="font-semibold text-white">Upload transcript files</h2>
          <span className="text-xs text-slate-500 ml-auto">.txt and .vtt only, max 10MB each</span>
        </div>
        ...
        >
          <input {...getInputProps()} />
          <Upload size={32} className={cn('mx-auto mb-3', isDragActive ? 'text-indigo-400' : 'text-slate-500')} />
          {isDragActive ? (
            ...
            {files.map((fw) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm text-white truncate">{fw.file.name}</div>
                  <div className="text-xs text-slate-500">{formatFileSize(fw.file.size)}</div>
                </div>

                {/* Status */}
                <div className="flex-shrink-0">
                  {fw.status === 'pending' && (
                    <span className="text-xs text-slate-400 bg-slate-700 px-2 py-0.5 rounded-full">Ready</span>
                  )}
                  {fw.status === 'uploading' && (
                  ...
                  {fw.status === 'error' && (
                    <div className="flex items-center gap-1 text-red-400">
                      <AlertCircle size={16} />
                      <span className="text-xs max-w-32 truncate">{fw.error}</span>
                    </div>
                  )}
                </div>
                ...
                {fw.status === 'success' && fw.result && (
                  <div className="text-xs text-slate-400 hidden sm:block">
                    {fw.result.speakerCount} speakers · {fw.result.wordCount.toLocaleString()} words · {fw.result.segmentCount} segments
                  </div>
                )}

                {fw.status !== 'uploading' && fw.status !== 'success' && (
                  <button onClick={() => removeFile(fw.file.name)} className="text-slate-500 hover:text-red-400 transition-colors ml-1">
                    <X size={14} />
                  </button>
                )}
              </div>
