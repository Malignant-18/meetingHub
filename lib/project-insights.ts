...
export type SpeakerSentimentSummary = {
  ...
  meetingCount: number
  distribution: Record<'positive' | 'neutral' | 'negative' | 'conflict', number>
}
...
const SENTIMENT_LABELS = ['positive', 'neutral', 'negative', 'conflict'] as const
...
export function summarizeProjectDecisions(
  ...
  const decisions = meetings
    ...
    .sort((a, b) => {
      const meetingDiff =
        new Date(b.meetingCreatedAt).getTime() - new Date(a.meetingCreatedAt).getTime()
  ...
  return {
    meetingsCovered: meetings.length,
...
export function summarizeProjectSentiment(
  ...
  const meetingSummaries = meetings
    ...
    .map((meeting) => {
      ...
      for (const sentiment of meeting.sentiments) {
        ...
        const existing =
          speakerMap.get(sentiment.speaker) ??
          {
            totalScore: 0,
            totalSegments: 0,
  ...
  const totalSegments = Object.values(distribution).reduce((sum, count) => sum + count, 0)
  const totalScore = meetings.reduce(
    (sum, meeting) =>
      sum + meeting.sentiments.reduce((inner, item) => inner + item.sentimentScore, 0),
    0
  )
  ...
  return {
    totalSegments,
    overallAverageScore: totalSegments > 0 ? round(totalScore / totalSegments) : null,
...
export function mapSentimentSegmentsToRecords(
  ...
  return analyzedSegments
    .map((item) => {
      const segmentId = selectBestSegmentId(transcriptSegments, item, usedSegmentIds)
...
function selectBestSegmentId(
  ...
  const candidates = speakerMatches.length > 0 ? speakerMatches : transcriptSegments
  ...
  for (const candidate of candidates) {
    ...
    if (normalizeText(candidate.speaker) === normalizeText(analyzedSegment.speaker)) {
      score += 0.25
    }
