...
import { ArrowLeft, FileText, Calendar, Users, AlignLeft, ChevronRight } from 'lucide-react'
...
async function getProject(projectId: string, clerkUserId: string) {
  ...
  return prisma.project.findFirst({
    ...
    include: {
      meetings: {
        include: {
          _count: { select: { actionItems: true, decisions: true, segments: true } },
        },
  ...
  return (
    <div className="min-h-screen bg-[#0f0e1a]">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        {/* Back + header */}
        <div className="mb-8">
          <Link href="/dashboard" className="flex items-center gap-1.5 text-slate-400 hover:text-white text-sm mb-4 transition-colors w-fit">
            <ArrowLeft size={14} />
            Back to dashboard
          </Link>
          <h1 className="text-2xl font-bold text-white">{project.name}</h1>
          <p className="text-slate-400 text-sm mt-1">
            {project.meetings.length} meeting{project.meetings.length !== 1 ? 's' : ''} · Created {formatDate(project.createdAt)}
          </p>
        </div>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium flex-shrink-0 ${statusColors[meeting.status]}`}>
              ...
              <div className="hidden sm:flex items-center gap-4 text-center flex-shrink-0">
                <div>
                  <div className="text-sm font-bold text-white">{meeting._count.segments}</div>
                  <div className="text-xs text-slate-500">segments</div>
                </div>
                <div>
                  <div className="text-sm font-bold text-white">{meeting._count.actionItems}</div>
                  <div className="text-xs text-slate-500">actions</div>
                </div>
                <div>
                  <div className="text-sm font-bold text-white">{meeting._count.decisions}</div>
        ...
        <div className="mt-12 grid sm:grid-cols-3 gap-4">
          {[
            { title: 'Decision Extractor', desc: 'Coming soon — AI extraction of all decisions across meetings', color: 'indigo' },
            { title: 'Sentiment Dashboard', desc: 'Coming soon — Visual tone analysis per speaker', color: 'purple' },
            { title: 'AI Chatbot', desc: 'Coming soon — Ask questions across all meetings', color: 'emerald' },
          ].map((panel) => (
            <div key={panel.title} className="bg-[#1e1c32] border border-slate-700/30 rounded-xl p-5 opacity-60">
              <div className="text-xs font-medium text-slate-500 uppercase tracking-wider mb-2">Coming soon</div>
              <h3 className="font-medium text-white mb-1">{panel.title}</h3>
